...
SELECT user_id, user_account, user_passowrd, user_salt
FROM `pre_go_acc_user_base_9999`
WHERE user_account = ?;

-- name: GetOneUserInfoAdmin :one
SELECT user_id, user_account, user_passowrd, user_salt, user_login_time, user_logout_time, user_login_ip, user_created_at, user_updated_at
FROM `pre_go_acc_user_base_9999`
WHERE user_account = ?;
...
INSERT INTO pre_go_acc_user_base_9999 (
    user_account, user_passowrd, user_salt, user_created_at, user_updated_at
) VALUES (
    ?, ?, ?, NOW(), NOW()
);
...
WHERE user_account = ? AND user_passowrd = ?;
